...
1. 結巴與重複：同一句話或同一段內容說了多次 → 只保留最完整流暢的版本，前後所有重複的都移除
   特別注意：講者可能連續嘗試好幾次才說完一句話，中間每次失敗的嘗試都算重複，即使接了不同的詞也一樣
   例如「第一個是的…第一個我們是的…第一個是我們台中市」→ 前兩次都要移除
2. 說到一半重來：講者說到某個地方卡住，重新從更前面開始講 → 移除不完整的部分，保留完整的
   即使不完整的部分後面緊接著新內容，只要語意上是在重來，就該移除
...
def main():
    ...
    if args.from_transcript:
        ...
        if not os.path.exists(json_path):
            print(f"\n   找不到逐字稿檔案：{json_path}")
            print(f"   請先不帶 --from-transcript 執行一次產出逐字稿，編輯後再載入。\n")
            sys.exit(1)
        ...
        words = transcribe(input_path, "medium")
    ...
    # --- Transcript output (skip when loading from existing transcript) ---
